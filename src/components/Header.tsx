...
export default function Header() {
  ...
  return (
    ...
    >
      ...
      >
        {isComponentVisible ? (
          <Cross1Icon className='w-5 h-5 transition-transform hover:scale-110' />
        ) : (
          <HamburgerMenuIcon className='w-5 h-5 transition-transform hover:scale-110' />
        )}
      </button>
      <Link href='/'>
      ...
      <div className='flex items-center gap-5'>
        <button onClick={() => setIsOpen(true)}>
          <BackpackIcon className='w-5 h-5 transition-transform hover:scale-110' />
        </button>
        <Link href='/login'>
          <PersonIcon className='w-5 h-5 transition-transform hover:scale-110' />
        </Link>
      </div>
